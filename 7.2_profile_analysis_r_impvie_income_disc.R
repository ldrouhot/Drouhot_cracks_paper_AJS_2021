...
labs(title="Income at 75th percentile, no discrimination", x= "", y = "Predicted religiosity: importance of religion in respondent's life (0-2 scale)", caption = "N.B: Horizontal line measures average predicted religiosity for natives") + geom_hline(data=muslim.profiles, aes(yintercept=native_ref_mean), color="red") + 
geom_pointrange() + theme(text = element_text(size=15)) 


print(profile.graph) # 

dev.print(pdf,'profile.graph.r_impvie_lower_income_no_disc.pdf')
