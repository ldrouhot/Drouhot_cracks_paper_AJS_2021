...
TeO.full <- TeO.full %>% filter(generation2!=1) # Filter for 1G/2G only and religious sign. Title of graph below needs adapting.
...
# then a "variable_draw" column.
	...
	model_99 <- lm(r_food ~ imm_rel_group_2 + imm_rel_group_3 + imm_rel_group_3 + imm_rel_group_4 + imm_rel_group_5 +
...
model_100 <- lm(formula = r_food ~  imm_rel_group_2 + imm_rel_group_3 + imm_rel_group_3 + imm_rel_group_4 + imm_rel_group_5 +
	sexee_1 +
	age +
	age_sq +
	f_dip +
...
coefficients<- coefficients %>%	mutate(term = replace(term, term == "xp_d_instit_1", "H2_xp_d_instit_1"))
...
coefficients<- coefficients %>%	mutate(term = replace(term, term == "imm_rel_group_2:xp_d_instit_1", "inter_muslim_H2_xp_d_instit_1"))
...
coefficients<- coefficients %>%	mutate(term = replace(term, term == "imm_rel_group_3:xp_d_instit_1", "inter_catholics_H2_xp_d_instit_1"))
...
coefficients<- coefficients %>%	mutate(term = replace(term, term == "imm_rel_group_4:xp_d_instit_1", "inter_christians_H2_xp_d_instit_1"))
...
coefficients<- coefficients %>%	mutate(term = replace(term, term == "imm_rel_group_5:xp_d_instit_1", "inter_non_religious_H2_xp_d_instit_1"))
...
coefficients_subset <- coefficients %>% filter(str_detect(term, 'H0_imm_rel_group') | #this keeps the dummies of interest only
											   ...
											   str_detect(term, 'd_perception') | str_detect(term, 'xp_interpersonal') | str_detect(term, 'xp_d_instit') |
...
{dwplot(coefficients_subset, dodge_size = 0.65, alpha=0.5,
   ...
   relabel_predictors(c( H0_imm_rel_group_2= "Muslim immigrants (ref: natives)",  # relabel predictors: main terms 
                         ...
                         H2_xp_d_instit_1 = "Reports institutional discrimination",
                         ...
                         inter_muslim_H2_xp_d_instit_1 = "Muslim * institutional discrimination",
                         ...
                         inter_catholics_H2_xp_d_instit_1 = "Catholic * institutional discrimination",
   ...
   ggtitle("Predicting subjective religiosity: following dietary constraints (0-2 scale) 2G only")  + theme_grey(base_size = 15) +
...
dev.print(pdf, 'full_model_r_food_2G_only')
