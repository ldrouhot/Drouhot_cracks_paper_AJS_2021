...
TeO.full <- TeO.full %>% filter(generation2!=1)		# filter to be adapted according to task at hand!										   	  
...
# then a "variable_draw" column.
	...
	model_99 <- lm(r_food ~  imm_rel_group +
	...
	low_religiosity_grid <- subsample_TeO.full %>% data_grid(cluster_assignment=c, ## First a grid for low predicted religiosity, setting key variables
		...
		t_d_neigh_maghreb_1=0, # the other dummies for high presence of SS African immigrants is set to modal values by default, which in this case is 0. 
		
		.model=model_99)

	mean_native_rel <- subsample_TeO.full %>% filter(imm_rel_group==1) %>% summarise(mean_native_rel=mean(r_food, na.rm = TRUE)) # generating mean native religiosity by iteration and cluster
...
labs(title="Income at 75th percentile", x= "", y = "Predicted religiosity: following dietary constraints (0-2 scale)", caption = "N.B: Horizontal line measures average predicted religiosity for natives") + geom_hline(data=muslim.profiles, aes(yintercept=native_ref_mean), color="red") + 
geom_pointrange() + theme(text = element_text(size=15)) 


print(profile.graph) # 

dev.print(pdf,'profile.graph.r_food_lower_income.pdf')
